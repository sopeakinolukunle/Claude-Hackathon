...
Examples:`;
  ...
  processedExamples.forEach((ex, i) => {
    ...
    } else if (ex.type === 'text' || !ex.type) {
      // Add text (default to text if type not specified)
      const textContent = typeof ex.content === 'string' ? ex.content : String(ex.content || '');
      if (textContent.trim()) {
        messageContent.push({ type: "text", text: `\nExample ${i + 1}:\n${textContent}\n` });
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const contents = await Promise.all(generationPromises);
